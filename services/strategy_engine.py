...
def compute_indicators(price_df: pd.DataFrame) -> dict:
    ...
    logger.debug(
        "[INDICATORS] Price: %.2f | SMA50: %.2f | SMA200: %.2f | RSI: %.2f",
        float(current_price),
        float(sma_50),
        float(sma_200),
        rsi_val,
...
def score_signal(indicators: dict, holding: dict) -> dict:
    ...
    return {
        ...
        "reasons": reasons,
        "indicators": {
            k: round(v, 2) if isinstance(v, float) else v for k, v in indicators.items()
        },
    }
    ...
    for h in holdings:
        ...
        if previous and previous.get("signal"):
            ...
            if curr_sig != prev_sig and abs(curr_score) < 0.7:
                ...
                sig_data["reasons"].insert(
                    0, f"Signal held at {prev_sig} (hysteresis prevents flip)"
                )
        ...
        # 5. Logging
        logger.debug(
            "[STRATEGY] %s | Score: %.2f | Signal: %s | Inds: %s",
            ticker,
            sig_data["score"],
            sig_data["signal"],
            sig_data["indicators"],
