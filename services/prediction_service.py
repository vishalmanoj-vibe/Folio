...
def get_forecast(dates: list, values: list, horizon_str: str, read_only: bool = False) -> dict:
    ...
    if not dates or not values:
        logger.warning(
            f"get_forecast called with empty data: dates={len(dates)}, values={len(values)}"
        )
        return {}
    ...
    try:
        row = conn.execute(
            "SELECT * FROM predictions_cache WHERE cache_key = ?", (cache_key,)
        ).fetchone()
    ...
    try:
        ...
        logger.info(
            f"Computing new prediction for {len(dates)} points. Horizon: {horizon_str} ({days} days)"
        )
        ...
        if original_count > 1260:
            ...
            logger.info(
                f"Sliced history to last 5 years ({len(df)} points) for better trend sensitivity."
            )
            original_count = len(df)
        ...
        try:
            ...
            if count_row and count_row[0] >= 20:
                oldest_key_row = conn.execute(
                    "SELECT cache_key FROM predictions_cache ORDER BY computed_at ASC LIMIT 1"
                ).fetchone()
                if oldest_key_row:
                    conn.execute(
                        "DELETE FROM predictions_cache WHERE cache_key = ?", (oldest_key_row[0],)
                    )
            ...
            """,
                (
                    ...
                    json.dumps(result["yhat"]),
                    json.dumps(result["yhat_lower"]),
                    json.dumps(result["yhat_upper"]),
                    result["fitted_last"],
                    result["computed_at"],
                ),
            )
