...
def record_snapshot(enriched_holdings: list[dict]):
    ...
    try:
        ...
        for h in enriched_holdings:
            ...
            row = conn.execute(
                "SELECT MAX(recorded_at) FROM intraday_snapshots WHERE ticker = ?",
                (ticker,)
            ).fetchone()
...
def clear_old_caches(keep_days: int = 2):
    ...
    cutoff = (pd.Timestamp.now(tz="Australia/Sydney") - pd.Timedelta(days=keep_days)).strftime("%Y-%m-%d")
    
    conn = get_connection()
    try:
        cursor = conn.execute(
            "DELETE FROM intraday_snapshots WHERE session_date < ?",
            (cutoff,)
        )
        conn.commit()
