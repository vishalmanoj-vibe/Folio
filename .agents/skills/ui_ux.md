...
## Rules of Engagement
- Never change a component ID — only style properties
- All colors via CSS variables only — never hardcode hex
- Test in both dark and light theme mentally before implementing
- Changes to layout.py must not alter the Dash component tree structure
- CSS goes in assets/styles.css — never inline in layout.py unless
  it's a dynamic value (e.g. driven by a callback)
