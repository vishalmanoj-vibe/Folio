...
### 7. `dcc.Loading` Spinner Never Activates
**Symptom**: A loading spinner added around a button never shows during a long-running callback.
**Cause**: `dcc.Loading` animates only when one of its **direct children** is the `Output` target of a callback. Wrapping the **Input** trigger (e.g. a button) has no effect — the spinner has no Output to observe.
**Solution**: Wrap a visible Output element instead. A common pattern is a status label sibling to the button:
```python
# ✅ Correct — spinner wraps the Output target
dmc.Button("Generate", id="my-btn"),
dcc.Loading(
    type="dot",
    children=html.Span(id="my-status-label")   # ← Output("my-status-label", "children")
)

# ❌ Wrong — spinner wraps the Input trigger, never fires
dcc.Loading(
    children=dmc.Button("Generate", id="my-btn")   # Input, not Output
)
```
...
### 8. `dmc.Button` — `leftIcon` / `rightIcon` Removed in v2
**Symptom**: `TypeError: The dash_mantine_components.Button component received an unexpected keyword argument: leftIcon`
**Cause**: `dash-mantine-components` v2.x renamed these props to align with the Mantine 7 API.
**Fix**:
```python
# ❌ Old (dmc v1)
dmc.Button("Label", leftIcon=html.I(className="fas fa-robot"))
