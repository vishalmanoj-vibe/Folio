...
**Theme**: Supporting Claude (Anthropic) and ChatGPT (OpenAI) alongside Gemini (Google) as selectable AI providers, with custom keys, dynamic model configurations, and complete integration in both Settings and Onboarding Setup Wizard.

*   **Unified AI Provider Gateway**: Created `services/ai_provider.py` which abstracts away the specific API/SDK models behind a simple, common interface (`generate_content`, `chat_completion`). It lazy-loads `openai` and `anthropic`, matches provider-model compatibility via `_resolve_model()`, and safely handles any API exceptions.
...
*   **Onboarding Setup Wizard Integration**: Replaced the static Gemini key input field on the Strategy & AI onboarding step (`pages/setup_ai.py`) with a full setup panel consisting of the AI Provider dropdown (defaulting to Gemini), Chat & Report Model selectors, an API key input, and a live connection validation tester.
