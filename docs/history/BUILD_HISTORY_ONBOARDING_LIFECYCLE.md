...
## Phase 13: Multi-Provider AI Model Selection (v2.8.0)
**Theme**: Supporting Claude (Anthropic) and ChatGPT (OpenAI) alongside Gemini (Google) as selectable AI providers, with custom keys and dynamic model configurations.

*   **Unified AI Provider Gateway**: Created `services/ai_provider.py` which abstracts away the specific API/SDK models behind a simple, common interface (`generate_content`, `chat_completion`). It lazy-loads `openai` and `anthropic` and safely handles any API exceptions.
*   **Settings UI Update**: Added AI Provider dropdown and API key configuration fields to the Settings page layout (`pages/settings.py`). 
*   **Dynamic Options & Prefilled Masked Keys**: Wrote Dash callbacks (`callbacks/settings_callbacks.py`) to dynamically filter model options based on the active provider (e.g. Claude Sonnet vs GPT-4o vs Gemini Flash) and prefill a masked `••••••••••••••••` API key representation if a key already exists in environment variables or database metadata.
*   **Test Connection Action**: Added a **Test Connection** button to the Settings page. This validates the API key live by issuing a tiny test completion request to the active provider and returns immediate color-coded success/failure feedback in the UI.
...
*   **Setup Integration**: Updated the onboarding setup callbacks to default the provider to `gemini` and persist key selections correctly in SQLite database metadata.
*   **Unit Test Suite Updates**: Updated the unit tests in `scratch/tests/test_ai_engine.py`, `scratch/tests/test_research.py`, and `scratch/tests/test_settings_callbacks.py` to match the refactored providers, dynamic options, and signature arguments.
*   **Files Changed**: `services/ai_provider.py` (new), `docs/guides/AI_PROVIDERS.md` (new), `pages/settings.py`, `callbacks/settings_callbacks.py`, `callbacks/setup_callbacks.py`, `services/ai_engine.py`, `services/research_service.py`, `services/sentiment_service.py`, `services/report_service.py`, `data/repository.py`, `data/settings_repository.py`, `config/settings.py`, `requirements.txt`, `.env.example`, `README.md`, `.agents/skills/registry.md`, `docs/reference/callback_ownership.md`, `scratch/tests/test_ai_engine.py`, `scratch/tests/test_research.py`, `scratch/tests/test_settings_callbacks.py`.
