...
| **`research_callbacks.py`** | AI Analyst & Reports (`/ai-analyst`) | Coordinates the interactive research chatbot interface, tracks rate limits, monitors memory, and manages weekly PDF report enqueues and downloads. |
...
*   **`ai_engine.py`**
    *   *Data Fetched/Computed:* Passes technical score signals to the Gemini API (`gemini-2.5-flash-lite`) to construct qualitative investment critiques, list structural asset risks, and normalize confidence verdicts.
