...
def build_performance_lollipops(
    data: list[dict], theme_tokens: dict, mode: str = "pct"
) -> go.Figure:
    """
    ...
    if not data:
        ...
        return create_empty_fig(
            "No performance data available", height=300, theme_tokens=theme_tokens
        )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            marker=dict(color=colors, size=10, line=dict(color=theme_tokens["BG"], width=1.5)),
            text=tickers,
            customdata=values,
