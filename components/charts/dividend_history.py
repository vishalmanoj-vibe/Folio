...
def build_portfolio_dividend_chart(df_full: pd.DataFrame, theme_tokens: dict) -> go.Figure:
    if df_full.empty:
        ...
        return create_empty_fig("No dividend history available", height=200, theme_tokens=theme_tokens)
    ...
    if df.empty:
        from components.charts.helpers import create_empty_fig
        return create_empty_fig("No dividends in the past year", height=200, theme_tokens=theme_tokens)
    ...
    fig.update_layout(
        ...
        title=dict(text="Portfolio Dividend Income (Past Year)", font=dict(size=12, color=theme_tokens["PLOTLY_BASE"]["font"]["color"])),
    )
