...
from config.constants import BORDER, COLORS, RED
...
def build_intel_geo_chart(geo_data: dict, theme_tokens: dict) -> go.Figure:
    ...
    if not geo_s:
        ...
        return create_empty_fig(
            "Geographic exposure data unavailable", height=200, bar=True, theme_tokens=theme_tokens
        )
    ...
    fig.add_trace(
        go.Bar(
            ...
            orientation="h",
            marker_color=[
                RED if v >= 60 else COLORS[2] if v >= 40 else COLORS[4] for _, v in geo_s
            ],
