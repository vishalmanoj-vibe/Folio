...
# ── Purchase history ───────────────────────────────────────────────────────────
# Load initial history from CSV
try:
    df = pd.read_csv("stock_portfolio_transactions.csv")
    if 'Type' in df.columns:
        df = df.rename(columns={'Type':'type', 'Ticker':'ticker', 'Shares':'shares', 'Price':'price', 'Date':'date'})
    else:
        df = df.rename(columns={'Ticker':'ticker', 'Shares':'shares', 'Price':'price', 'Date':'date'})
        df['type'] = 'buy'
    df['date'] = pd.to_datetime(df['date'], format='%d.%m.%Y').dt.strftime('%Y-%m-%d')
    INITIAL_HISTORY = df.to_dict('records')
    print(f"Loaded {len(INITIAL_HISTORY)} transactions from CSV")
...
def add_transaction(n, txn_type, ticker, shares, price, date, history):
    ...
    try:
        ...
        df = df[['type', 'ticker', 'shares', 'price', 'Date']].rename(columns={'type':'Type', 'ticker':'Ticker', 'shares':'Shares', 'price':'Price', 'Date':'Date'})
        df.to_csv("stock_portfolio_transactions.csv", index=False)
