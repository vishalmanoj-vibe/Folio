...
from components.ui_helpers import interpolate_color, progress_row, stat_card, tech_signal_badges
from config.constants import BG, BORDER, COLORS, GREEN, NAMES, RED, SURFACE, T_PRI, T_SEC, get_theme
...
def register_callbacks(app) -> None:
    ...
    def render_card_grid(port_data, selected_ticker, url_pathname, signals_store):
        ...
        if url_pathname.rstrip("/") != "/positions":
        ...
        for h in port_data["holdings"]:
            try:
                ...
                card_children = [
                    ...
                    html.Div(
                        f"${h.get('mkt_value') or h.get('total_cost', 0.0):,.2f}",
                        className="holding-card-value",
                    ),
                    html.Div(f"{pnl:+.2f}%", className=f"holding-card-pnl {pnl_cls}"),
                    html.Div(
                        f"Yield: {h.get('div_yield', 0):.2f}%",
                        className="holding-card-name",
                        style={"marginTop": "4px"},
                    ),
                ]
    ...
    def select_ticker(n_clicks_list, port_data, current):
        ...
        if (
            ...
            and ctx.triggered_id.get("type") == "pos-card"
        ):
    ...
    def render_detail_metrics(ticker, port_data, url_pathname):
        ...
        if not h:
            return html.Div(
                f"Metrics for {ticker} are currently unavailable", className="c-muted"
            ), None
        ...
        cards_layout = [
            html.Div(
                [
                    ...
                    html.Div(
                        val, className="etf-detail-value", style={"color": color} if color else {}
                    ),
            ...
            for label, val, sub, color in [
                (m[0], m[1], m[2], m[3] if len(m) > 3 else None) for m in metrics
            ]
        ]
    ...
    def render_ai_insight(ticker, signals_store, url_pathname):
        ...
        ai_children = [
            ...
            html.Div(
                verdict, className="etf-detail-value", style={"color": v_color, "fontSize": "16px"}
            ),
            html.Div(
            ...
            ai_children.append(
                html.Div(
                    [
                        html.Div(
                            f"• {r}",
                            style={"color": "var(--red)", "marginTop": "4px", "fontSize": "12px"},
                        )
        ...
        try:
            ...
            if not history_records:
                fig = create_empty_fig(
                    f"No price history for {ticker}", height=350, theme_tokens=t_
                )
            else:
                import pandas as pd
                ...
                if not df.empty:
                    ...
                    fig = create_empty_fig(
                        f"No price history available for {ticker}", height=350, theme_tokens=t_
                    )
        except Exception as e:
    ...
    def render_txn_table(ticker, history, url_pathname):
        ...
        if not ticker or not history:
            return None
        txns = sorted(
            [t for t in history if t["ticker"].upper() == ticker],
            key=lambda x: x["date"],
            reverse=True,
        )
        if not txns:
            ...
            rows.append(
                html.Tr(
                    [
                        ...
                        html.Td(
                            t["type"].upper(),
                            style={**_TD, "color": GREEN if is_buy else RED, "fontWeight": "600"},
                        ),
                    ...
                    [
                        html.Thead(
                            html.Tr(
                                [
                                    html.Th(c, style=_TH)
                                    for c in ["Date", "Type", "Shares", "Price", "Total"]
                                ]
                            )
                        ),
                        html.Tbody(rows),
    ...
    def render_period_btns(current_period, ticker, url_pathname):
        ...
        periods = [("1M", "1mo"), ("3M", "3mo"), ("1Y", "1y"), ("YTD", "ytd"), ("MAX", "max")]
    ...
    def render_ticker_dividends(ticker, port_data, theme, url_pathname, txn_data):
        ...
        if df.empty:
            return html.Div(
                "No dividend history found for this position.",
                className="c-muted",
                style={
                    "fontSize": "13px",
                    "padding": "20px",
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                visible=True, showgrid=False, tickformat="%b %y", tickfont=dict(size=9, color=T_SEC)
            ),
            yaxis=dict(visible=False),
        )
            ...
            rows.append(
                html.Tr(
                    [
                        ...
                        html.Td(
                            row["pay_date"] if row["pay_date"] else "—",
                            style={**_TD, "color": T_SEC},
                        ),
                        html.Td(
                            f"${row['amount']:.4f}",
                            style={**_TD, "fontWeight": "600", "color": GREEN},
                        ),
                    ]
                )
            )
        ...
        table = html.Table(
            [
                html.Thead(
                    html.Tr([html.Th(c, style=_TH) for c in ["Ex-Date", "Pay-Date", "Amount"]])
                ),
                html.Tbody(rows),
            ],
        ...
        return html.Div(
            [
                chart_title("Dividend trend & history"),
                html.Div(
                    [
                        html.Div(
    ...
    def render_portfolio_dividend_insights(port_data, theme, url_pathname):
        ...
        income_data = sorted(
            [{"ticker": h["ticker"], "val": h.get("annual_div", 0)} for h in holdings],
            key=lambda x: x["val"],
            reverse=True,
        )
        ...
        income_rows = [
            progress_row(
                ...
                max_income,
                prefix="$",
                color=interpolate_color(C_START, C_END, i / (n_income - 1))
                if n_income > 1
                else C_START,
            )
        ...
        yield_data = sorted(
            [{"ticker": h["ticker"], "val": h.get("div_yield", 0)} for h in holdings],
            key=lambda x: x["val"],
            reverse=True,
        )
        ...
        yield_rows = [
            progress_row(
                ...
                max_yield,
                suffix="%",
                color=interpolate_color(C_START, C_END, i / (n_yield - 1))
                if n_yield > 1
                else C_START,
            )
                ...
                rows.append(
                    html.Tr(
                        [
                            ...
                            html.Td(
                                f"${row['total']:,.2f}",
                                style={**_TD, "color": GREEN, "fontWeight": "600"},
                            ),
                        ]
                    )
                )
