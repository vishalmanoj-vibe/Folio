...
from components.charts.intel_helpers import _BAR_MIN_H, create_empty_fig
from components.ui_helpers import alert_card, stat_card
...
def register_callbacks(app) -> None:
    ...
    def update_intelligence(
        ...
        try:
            ...
            if url_pathname.rstrip("/") != "/intelligence":
            ...
            logger.info(
                f"Intelligence update triggered. pred_on={pred_on}, period={period}, path={url_pathname}"
            )
            ...
            try:
                ...
                if bench_data is None:
                    ...
                    try:
                        ...
                        if row:
                            ...
                            fetch_period = (
                                get_earliest_purchase_date() if period == "max" else period
                            )
                ...
                metrics = compute_risk_metrics(
                    port_data, period=period, returns=full_returns, histories=histories
                )
                vol = metrics["vol"]
                ...
                vol_c = (
                    RED
                    if not _nan(vol) and vol > 20
                    else COLORS[2]
                    if not _nan(vol) and vol > 12
                    else GREEN
                )
                shr_c = (
                    GREEN
                    if not _nan(sharpe) and sharpe > 1
                    else COLORS[2]
                    if not _nan(sharpe) and sharpe > 0.5
                    else RED
                )
                dd_c = (
                    RED
                    if not _nan(max_dd) and max_dd < -15
                    else COLORS[2]
                    if not _nan(max_dd) and max_dd < -8
                    else GREEN
                )
                cdd_c = (
                    RED
                    if not _nan(cur_dd) and cur_dd < -10
                    else COLORS[2]
                    if not _nan(cur_dd) and cur_dd < -5
                    else GREEN
                )
                ...
                b_traces = (
                    build_benchmark_traces(
                        period, t_, portfolio_start=p_start, benchmarks=bench_data
                    )
                    if bench_data
                    else []
                )

                eq_fig = build_intel_equity_chart(ret_dates, ret_values, t_, benchmarks=b_traces)
                eq_fig.update_layout(uirevision=f"pred_{pred_on}")

                # ── B2. Prediction Trace (Optional) ───────────────────────────────────
                if pred_on:
                    full_metrics = compute_risk_metrics(
                        port_data, period="max", returns=full_returns
                    )
                    ...
                    if pred_data is None:
                        ...
                        try:
                            ...
                            cursor = conn.execute(
                                "SELECT payload FROM worker_tasks WHERE task_type = 'generate_prediction' AND status IN ('pending', 'running')"
                            )
                            ...
                            if not already_tasked:
                                ...
                                logger.info(
                                    f"Enqueued background prediction task for horizon: {target_horizon}"
                                )
                        ...
                        logger.info(
                            f"Adding forecast trace to equity chart. Points: {len(pred_data['dates'])}"
                        )
                        ...
                        chart_last_v = (
                            metrics.get("ret_values", [])[-1] if metrics.get("ret_values") else 0
                        )
                        ...
                        last_d = (
                            metrics.get("ret_dates", [])[-1] if metrics.get("ret_dates") else None
                        )
                ...
                dd_fig = build_intel_drawdown_chart(
                    metrics.get("dd_dates", []), metrics.get("dd_values", []), t_
                )
                dd_fig.update_layout(uirevision=period)

                # ── G. Smart alerts ───────────────────────────────────────────────────
                alert_cards = [alert_card(a) for a in compute_smart_alerts(metrics, port_data)]
                ...
                # 4. intel-alerts
                # 5. intel-data-note
                return (risk_cards, eq_fig, dd_fig, alert_cards, data_note, new_bench_pending)
            except Exception:
