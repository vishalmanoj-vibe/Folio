    ...
    def trigger_global_intelligence(n, port_data, watch_data, port_signals, watch_signals, pending):
        ...
        if p_tickers and port_signals and port_signals.get("generated_at"):
            if port_signals["generated_at"].startswith(today_str):
                ...
                if all(t in cached for t in p_tickers):
                    has_errors = any(
                        "No market data" in " ".join(raw_data.get(t, {}).get("reasons", []))
                        or "Insufficient data" in " ".join(raw_data.get(t, {}).get("reasons", []))
                        or raw_data.get(t, {}).get("score", 0.0) == 0.0
                        for t in p_tickers
                    )
                    if not has_errors:
        ...
        if p_tickers and p_needs_update:
            task_id = enqueue_task(
                "generate_signals", {"tickers": p_tickers, "scope": "portfolio"}, priority=3
            )
        ...
        if w_tickers and w_needs_update:
            task_id = enqueue_task(
                "generate_signals", {"tickers": w_tickers, "scope": "watchlist"}, priority=3
            )
    ...
    def poll_tasks_and_update_stores(n, pending, port_data, watch_data):
        ...
        try:
            ...
            for task in pending:
                task_id = task["id"]
                row = conn.execute(
                    "SELECT status FROM worker_tasks WHERE task_id = ?", (task_id,)
                ).fetchone()
...
def _load_signal_results(tickers: list[str], table: str = "signal_results") -> dict:
    ...
    try:
        ...
        cursor = conn.execute(
            f"SELECT * FROM {table} WHERE ticker IN ({placeholders})", [t.upper() for t in tickers]
        )
