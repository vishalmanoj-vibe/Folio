...
> Claude Code entry point. Mirrors [`GEMINI.md`](GEMINI.md) — keep the two in sync when rules change (with explicit user approval).
> Supporting material lives in `.agents/`: [`skills/`](.agents/skills/) (tested patterns, start at [`registry.md`](.agents/skills/registry.md)), [`workflows/`](.agents/workflows/startcycle.md), [`agents.md`](.agents/agents.md), [`production_artifacts/`](.agents/production_artifacts/) (specs, `task.md`, `build_log.md`).
...
5. **This file ([`CLAUDE.md`](CLAUDE.md))** — Architecture rules. Read the relevant section for what you're changing.
...
- `CLAUDE.md` itself (and its mirror `GEMINI.md`) — architectural rules require human judgment.
